...
int col_jes[njes] = {kViolet+2,2,41,4,5,6,7,kGreen+1,9,11,kCyan+2,kCyan-10, kBlue-7, kMagenta-9, kRed-5,40,kTeal+3,42,43,44,45,46,28,32,36,49,50,51};
...
//	sprintf(name,"BbB_unfold_%s_Etabin%i_JES%i",JetRad[ij],jk+1,kl);
	sprintf(name,"mcjets/JetpT_%s_EtaBin%i_JES%i",JetRad[ij],jk+1,kl);
	if(kl>njes) {
//	sprintf(name,"BbB_unfold_%s_Etabin%i_JES%i",JetRad[ij],jk+1,kl+9);
...
//       sprintf(name,"BbB_unfold_%s_Etabin%i_JES37",JetRad[ij],jk+1);
	   sprintf(name,"mcjets/JetpT_%s_EtaBin%i_JES37",JetRad[ij],jk+1);
       Jet_pt_tot[ij][jk][0] = (TH1D*)file0->Get(name);

//       sprintf(name,"BbB_unfold_%s_Etabin%i_JES74",JetRad[ij],jk+1);
...
for(int kl=0; kl<njes; kl++){
  ...
  leg_pt[ij][jk]->AddEntry(JetpT_jes_up_fac[ij][jk][kl],jes_name[kl],"l");
 ...
 leg_pt[ij][jk]->AddEntry(JetpT_jes_up_sum[ij][jk],"Total","l");
 ...
 */
 ...
 sprintf(name,"JES_Plots/Pt_Plots/Factorised/Legacy2016/BeforeUnfolding/%s.png",c2[ij][jk]->GetName());
...
//  latex.DrawLatex(0.35,0.65,text2);
 ...
 for(int kl=0; kl<njes; kl++){
  ...
  leg_rad[ij][jk]->AddEntry(JetRad_jes_up_fac[ij][jk][kl],jes_name[kl],"l");
 ...
 leg_rad[ij][jk]->AddEntry(JetRad_jes_up_sum[ij][jk],"Total","l");
 ...
 */
 ...
 sprintf(name,"JES_Plots/Ratio_Plots/Factorised/Legacy2016/BeforeUnfolding/%s.png",c3[ij][jk]->GetName());
