...
const char *filenames_rad[nfiles] = 
   {"Theory_Unc_Files/Scale_wAK4_Rad.log",
	"Theory_Unc_Files/MPI_Rad.log",
	"Theory_Unc_Files/NP_Total_wAK4_Rad.log",
	"Theory_Unc_Files/PS/PSVar_PHP8_Rad_LogLinear.log"};
	

ofstream fp_pt_tot;
fp_pt_tot.open("Total_Theory_Unc_Pt_Small_84.log");

ofstream fp_rad_tot;
fp_rad_tot.open("Total_Theory_Unc_Rad_Small_84.log");

int col[nfiles] = {1,7,9,kYellow+3};
int sty[nfiles] = {1,5,9,7};
const char *sys_name[nfiles] = {"Scale","MPI","NP Correction","Parton Shower"};
...
float radlimd[netarange] = {0.105,0.105,0.105,0.3,0.32,0.4,0.4,0.4,0.4,0.4};
  ...
  for(int jk=0; jk<netarange; jk++){
   ...
   for(int kl=0; kl<noptbins; kl++){
	...
	if(pt_err_up[fg][ij][jk][kl]>0.9999) {pt_err_up[fg][ij][jk][kl] = 0.;}
	   ...
	   if(fg==2 && pt_err_up[fg][ij][jk][kl]>0.0601) { pt_err_up[fg][ij][jk][kl] = 0.0599;}
	   if(fg==2 && pt_err_dn[fg][ij][jk][kl]>0.0601) { pt_err_dn[fg][ij][jk][kl] = 0.0599;}
	...
	if(rad_err_up[fg][ij][jk][kl]>0.9999) {rad_err_up[fg][ij][jk][kl] = 0.;}
      ...
      hist_rad_up[fg][ij][jk]->GetXaxis()->SetTitleOffset(0.75);
...
//      hist_rad_up[fg][ij][jk]->GetYaxis()->SetNdivisions(406);
      hist_rad_up[fg][ij][jk]->GetYaxis()->SetTitleOffset(1.75);
 ...
 for(int ij=0; ij<nalgo; ij++){
  ...
  sprintf(name,"Theory_Sys_Plots_Small/Pt_Plots/%s.pdf",c2[ij][jk]->GetName());
  c2[ij][jk]->SaveAs(name);

  sprintf(name,"Tot_rad_err_%s_EtaBin%i",JetRad[ij],jk+1);
  c3[ij][jk] =  tdrCanvas(name,hist_rad_up[0][ij][jk],2,0);

  leg_rad[ij][jk] =  new TLegend(leg_x1-0.285,leg_y1+0.05,leg_x2+0.01,leg_y2);
...
//  latex.DrawLatex(0.5*(leg_x1+leg_x2)+0.325,leg_y1+0.125,text1);
  ...
  for(int fg=0; fg<nfiles; fg++){
   ...
   hist_rad_up[fg][ij][jk]->GetYaxis()->SetTitleOffset(1.5);
   hist_rad_dn[fg][ij][jk]->GetYaxis()->SetTitleOffset(1.5);
  ...
  sprintf(name,"Theory_Sys_Plots_Small/Ratio_Plots/%s.pdf",c3[ij][jk]->GetName());
