...
void Compare_Unf_Algo()
{
const int nfiles = 3;
const int nmc = 3;
/*
const char *filenames[nfiles] = {"/home/suman/RooUnfold/Unfolded_2016_Data_Hwg_MC_P8_Legacy_JECv10_excor.root",
				 "/home/suman/RooUnfold/Unfolded_2016_Data_Hwg_MC_P8_Legacy_JECv10_excor.root",
				 "/home/suman/RooUnfold/Unfolded_2016_Data_Hwg_MC_P8_Legacy_JECv10_excor.root"
				};
*/
...
const char *model_name[nfiles] = {"D'Agostini","SVD","Bin by Bin"};
...
ifstream fp_pt("/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Systematics/RM_Unc_Pt_P8.log");
ifstream fp_rad("/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Systematics/RM_Unc_Rad_P8.log");

for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
   for(int bn=0; bn<noptbins; bn++){

   fp_pt>>xx1>>xx2>>xx3>>pt_err[ij][jk][bn]>>pt_err[ij][jk][bn];
   fp_rad>>xx1>>xx2>>xx3>>rad_err[ij][jk][bn]>>rad_err[ij][jk][bn];   
...
//    cout<<"finally "<<hist_pt[fg][ij][jk]->GetBinError(bn+1)*1./hist_pt[fg][ij][jk]->GetBinContent(bn+1)<<endl;
    ...
    hist_pt[fg][ij][jk]->GetXaxis()->SetRangeUser(84,3000.);
...
for(int fg=0; fg<nfiles; fg++){
 for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
   ...
   hist_radius[fg][ij][jk]->GetXaxis()->SetRangeUser(84,3000.);
...
for(int ij=0; ij<nalgo; ij++){
 ...
 leg_pt[ij][jk] = tdrLeg(0.6,0.7,0.85,0.9);
 ...
 for(int fg=0; fg<nfiles; fg++){
      ...
      if(fg!=0){
		...
		hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("MC / D'Agostini");
...
for(int fg=0; fg<(nfiles-1); fg++){
 ...
 sprintf(name,"Unfolding_Unc_Plots/JECv10/Method/Pt_Plots/%s.png",c2[ij][jk]->GetName());
...
TLegend *leg_radius[nalgo][netarange];
 ...
 if((ij<2)&&(jk>6)) continue;
  ...
  leg_radius[ij][jk] = tdrLeg(0.6,0.7,0.85,0.9);
  ...
  for(int fg=0; fg<nfiles; fg++){
      ...
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("MC / D'Agostini");
      ...
      for(int bn=0; bn<(hist_radius_rat[fg-1][ij][jk]->GetNbinsX()); bn++){
                        hist_radius_rat[fg-1][ij][jk]->SetBinError(bn+1,(1./sqrt(2))*hist_radius_rat[fg-1][ij][jk]->GetBinError(bn+1));
                }
  ...
  sprintf(name,"Unfolding_Unc_Plots/JECv10/Method/Ratio_Plots/%s.png",c3[ij][jk]->GetName());
