...
void Compare_JERVar_RECO()
{
const int nfiles = 3;

//const char *filenames[nfiles] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Histogram_P8_Noswap_FullPU_2.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Histogram_P8_Noswap_FullPU_2.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Histogram_P8_Noswap_FullPU_2.root"};
const char *filenames[nfiles] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Hist_Pythia_ntuple_OneReso.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Hist_Pythia_ntuple_OneReso.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Hist_Pythia_ntuple_OneReso.root"};
...
for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
   ...
   for(int bn=0; bn<(hist_pt[fg][ij][jk]->GetNbinsX()); bn++){
    ...
    hist_pt[fg][ij][jk]->SetMaximum(hist_pt[fg][ij][jk]->GetMaximum());
  }//jk
 }//ij
}//fg
...
for(int ij=0; ij<nalgo; ij++){
 ...
 sprintf(name,"JER_Compare_Canv_%s_EtaBin%i",JetRad[ij],jk+1);
 c2[ij][jk] = tdrDiCanvas(name,hist_pt[0][ij][jk],hist_pt[1][ij][jk],7,0);

 leg_pt[ij][jk] = tdrLeg(0.6,0.7,0.8,0.9);
 ...
 gPad->SetLogy(1);
  ...
  latex.DrawLatex(0.5*(leg_x1+leg_x2)+0.1,leg_y1-0.12,text1);
 ...
 for(int fg=0; fg<nfiles; fg++){
      ...
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("JER Varied / Nominal");
...
for(int fg=0; fg<(nfiles-1); fg++){
   ...
   for(int bn=0; bn<(hist_pt_rat[fg][ij][jk]->GetNbinsX()); bn++){
	hist_pt_rat[fg][ij][jk]->SetBinError(bn+1,0);
	}
...
sprintf(name,"JER_RECO_Plots/Pt_Plots/%s.eps",c2[ij][jk]->GetName());
...
for(int ij=0; ij<nalgo; ij++){
 ...
 if((ij<2)&&(jk>6)) continue;
  ...
  sprintf(name,"JER_Ratio_Compare_Canv_%s_EtaBin%i",JetRad[ij],jk+1);
  c3[ij][jk] = tdrDiCanvas(name,hist_radius[0][ij][jk],hist_radius[1][ij][jk],7,0);

  leg_radius[ij][jk] = tdrLeg(0.6,0.7,0.8,0.9);
  ...
  for(int fg=0; fg<nfiles; fg++){
      ...
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("JER Varied / Nominal");
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitleSize(0.085);
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitleOffset(0.9);
  ...
  sprintf(name,"JER_RECO_Plots/Ratio_Plots/%s.eps",c3[ij][jk]->GetName());
