...
for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
    for(int pt=0; pt<noptbins; pt++){
     hist_pt_data[ij][jk]->SetBinContent(pt+1,hist_pt_mc[ij][jk]->GetBinContent(pt+1));
     hist_pt_data[ij][jk]->SetBinError(pt+1,hist_pt_mc[ij][jk]->GetBinError(pt+1));
    }
  }
}
...
for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
	...
	TAxis *axis = hist_pt_mc[ij][jk]->GetXaxis();
    int amin =0, amax =0;
	  
	for(int kl=0; kl<nopbins; kl++){
        amin = axis->FindBin(pbins[kl]);
        amax = axis->FindBin(pbins[kl+1]);
        xsection[ij][jk][kl] = hist_pt_mc[ij][jk]->Integral(amin,amax,"width");
	}
  }
}
...
for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
    for(int pt=0; pt<noptbins; pt++){
     hist_pt_data[ij][jk]->SetBinContent(pt+1,hist_pt_mc[ij][jk]->GetBinContent(pt+1) * hist_pt_mc[ij][jk]->GetBinWidth(pt+1));  //MC
     hist_pt_data[ij][jk]->SetBinError(pt+1,hist_pt_mc[ij][jk]->GetBinError(pt+1) * hist_pt_mc[ij][jk]->GetBinWidth(pt+1));	//MC
    }
    hist_pt_data[ij][jk]->Scale(1./1000); // fb to pb
  }
}

TFile *file_out = new TFile("pt_spectra_unfolded_2016_pythia.root","RECREATE");
