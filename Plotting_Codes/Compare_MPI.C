...
const char *filenames[nfiles] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pure_Gen/Powheg/Powheg_Pythia/PDF_NNPDF30/hist_Powheg_Pythia_Userhook2_NNPDF30.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pure_Gen/Powheg/Powheg_Pythia/PDF_NNPDF30/hist_Powheg_PythiaNNPDF30_Up.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pure_Gen/Powheg/Powheg_Pythia/PDF_NNPDF30/hist_Powheg_PythiaNNPDF30_Dn.root"};

const int ref = 3;

const char *model_name[nfiles] = {"MPI_Nominal","MPI_Up", "MPI_Down"};
...
ofstream fp_pt;
fp_pt.open("MPI_Pt.log");

ofstream fp_rad;
fp_rad.open("MPI_Rad.log");
...
*/
 ...
 for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
   ...
   hist_radius[fg][ij][jk]->GetYaxis()->SetLabelSize(0.04);
...
//      hist_pt[fg][ij][jk]->Scale(1./hist_pt[fg][ij][jk]->Integral());
      ...
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("Ratio to MPI_Nominal");
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitleSize(0.0825);
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
 ...
 line->Draw();
  ...
  sprintf(name,"MPI_Var/Pt_Plots/%s.eps",c2[ij][jk]->GetName());
...
//  if((ij!=ieta)|(jk!=ialgo)) continue;
  ...
  for(int fg=0; fg<nfiles; fg++){
      ...
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("Ratio to MPI_Nominal");
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
  ...
  sprintf(name,"MPI_Var/Ratio_Plots/%s.eps",c3[ij][jk]->GetName());
...
for(int ij=0; ij<nalgo; ij++){
 for(int jk=0; jk<netarange; jk++){
   ...
   double bs[nfiles] = {0};
	...
	if(hist_pt_rat[0][ij][jk]->GetBinError(bn+1)>pt_err_up[ij][jk][bn]) { pt_err_up[ij][jk][bn] = 0;}
	if(hist_pt_rat[0][ij][jk]->GetBinError(bn+1)>pt_err_dn[ij][jk][bn]) { pt_err_dn[ij][jk][bn] = 0;}

	//cout<<ij<<'\t'<<jk<<endl;
	if(!(ij<2&&jk>6)){ 
	if(hist_radius_rat[0][ij][jk]->GetBinError(bn+1)>rad_err_up[ij][jk][bn]) { rad_err_up[ij][jk][bn] = 0;}
        if(hist_radius_rat[0][ij][jk]->GetBinError(bn+1)>rad_err_dn[ij][jk][bn]) { rad_err_dn[ij][jk][bn] = 0;}
	}
