...
const char *filenames[nfiles] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Data_2016/Legacy/hist_Legacy_2016_withMETcut.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Pythia_NTuples_LegacyCor.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Data_2016/NTuples/2016BCDEFGH_ntuple_nocor.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/P8_FullPU_tworeso.root"};

double data_lumi = 1.;

data_lumi = 33.6;//8.762;//8.762;//8.762;//8.762;//8.762;//8.762;//8.762;//27.683;
data_lumi = 32.1;

const int ref = 3;

const char *model_name[nfiles] = {"Data (Legacy)","P8 CUETP8M1 (Legacy)","Data (reReco)","P8 CUETP8M1"};
const int col[nfiles] = {1,6,4,2};

const char *legacy_name[nfiles] = {"Legacy","reReco"};
 ...
 for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){
    ...
    if(jk==5&&ij==1) { cout<<filenames[fg]<<" "<<hist_pt_org[fg][ij][jk]->GetName()<<" mean "<<hist_pt_org[fg][ij][jk]->GetMean()<<endl;}
     ...
     if(fg==2){hist_pt_org[fg][ij][jk]->Scale(1./31.9);}
...
leg_pt_rat[ij][jk] = tdrLeg(0.5,0.235,0.85,0.285);
...
//	hist_pt[fg][ij][jk]->Scale(1./hist_pt[fg][ij][jk]->Integral());
      ...
      hist_pt_rat[fg/2][ij][jk] = (TH1D*)hist_pt[fg][ij][jk]->Clone();
      hist_pt_rat[fg/2][ij][jk]->Divide(hist_pt[2*(fg/2)][ij][jk]);
      hist_pt_rat[fg/2][ij][jk]->GetYaxis()->SetTitle("MC/Data");
      hist_pt_rat[fg/2][ij][jk]->GetYaxis()->SetTitleSize(0.1);
      hist_pt_rat[fg/2][ij][jk]->GetYaxis()->SetLabelSize(0.075);
      hist_pt_rat[fg/2][ij][jk]->GetYaxis()->SetTitleOffset(0.7);
      hist_pt_rat[fg/2][ij][jk]->GetXaxis()->SetTitleSize(0.115);
      hist_pt_rat[fg/2][ij][jk]->GetXaxis()->SetLabelSize(0.085);
      hist_pt_rat[fg/2][ij][jk]->GetXaxis()->SetTitleOffset(0.9);
      cout<<model_name[fg]<<" "<<ij<<" "<<jk<<" delta "<<Delta(hist_pt[0][ij][jk],hist_pt[fg][ij][jk])<<endl ;
      delta_pt[fg/2][ij][jk] = Delta(hist_pt[0][ij][jk],hist_pt[fg][ij][jk]) ;
	}
...
if(fg==0) {  hist_pt_rat[fg][ij][jk]->Draw("e"); }
 ...
 leg_pt_rat[ij][jk]->AddEntry(hist_pt_rat[fg][ij][jk],legacy_name[fg],"l");
 }

 leg_pt_rat[ij][jk]->SetTextSize(0.03);
 leg_pt_rat[ij][jk]->SetTextFont(42);

 line->Draw();

sprintf(name,"Legacy_Cor/With_MC/Pt_Plots/%s.eps",c2[ij][jk]->GetName());
...
//  if((ij!=ieta)|(jk!=ialgo)) continue;
  ...
  leg_radius_rat[ij][jk] = tdrLeg(0.5,0.235,0.85,0.285);
  ...
  for(int fg=0; fg<(nfiles); fg++){
      if((fg%2)!=0){
      hist_radius_rat[fg/2][ij][jk] = (TH1D*)hist_radius[fg][ij][jk]->Clone();
      hist_radius_rat[fg/2][ij][jk]->Divide(hist_radius[2*(fg/2)][ij][jk]);
      hist_radius_rat[fg/2][ij][jk]->GetYaxis()->SetTitle("MC/Data");
      hist_radius_rat[fg/2][ij][jk]->GetYaxis()->SetTitleSize(0.1);
      hist_radius_rat[fg/2][ij][jk]->GetYaxis()->SetLabelSize(0.075);
      hist_radius_rat[fg/2][ij][jk]->GetYaxis()->SetTitleOffset(0.7);
      hist_radius_rat[fg/2][ij][jk]->GetXaxis()->SetTitleSize(0.115);
      hist_radius_rat[fg/2][ij][jk]->GetXaxis()->SetLabelSize(0.085);
      hist_radius_rat[fg/2][ij][jk]->GetXaxis()->SetTitleOffset(0.9);


      hist_radius_rat[fg/2][ij][jk]->SetMinimum(0.2);
      hist_radius_rat[fg/2][ij][jk]->SetMaximum(2.0);

      hist_radius_rat[fg/2][ij][jk]->GetXaxis()->SetRangeUser(min_range,max_range);

      delta_radius[fg/2][ij][jk] = Delta(hist_radius[0][ij][jk],hist_radius[fg][ij][jk]);
 ...
 // leg_radius[ij][jk]->Draw();
   ...
   for(int fg=0; fg<(nfiles/2); fg++){
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
   hist_radius_rat[fg][ij][jk]->SetLineColor(col[2*fg]);
   if(fg==0){
         hist_radius_rat[fg][ij][jk]->Draw("e");
   }else{
         hist_radius_rat[fg][ij][jk]->Draw("sames");
	}
   leg_radius_rat[ij][jk]->AddEntry(hist_radius_rat[fg][ij][jk],legacy_name[fg],"l");
   }
  ...
  sprintf(name,"Legacy_Cor/With_MC/Ratio_Plots/%s.eps",c3[ij][jk]->GetName());
