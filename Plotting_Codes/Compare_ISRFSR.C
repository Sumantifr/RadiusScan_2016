...
const char *model_name[nfiles] = {"Nominal","ISR off","FSR off"};
...
c2[ij][jk]->cd(1);
 ...
 gPad->SetLogy(1);
  ...
  latex.DrawLatex(0.5*(leg_x1+leg_x2)+0.15,leg_y1+0.025,"Anti k_{T} CHS");
...
//	hist_pt[fg][ij][jk]->Scale(1./hist_pt[fg][ij][jk]->Integral());
      ...
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("MC/Nominal");
      ...
      tdrDraw(hist_pt[fg][ij][jk],algo_style[fg],col[fg],line_sty[fg],col[fg] ,1001,2);
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
  ...
  hist_pt_rat[fg][ij][jk]->SetLineStyle(line_sty[fg+1]);
  hist_pt_rat[fg][ij][jk]->SetMarkerColor(col[fg+1]);
  hist_pt_rat[fg][ij][jk]->SetMarkerStyle(algo_style[fg+1]);
...
sprintf(name,"ISR_FSR_Plots/Pt_Plots/%s.pdf",c2[ij][jk]->GetName());
...
//  if((ij!=ieta)|(jk!=ialgo)) continue;
  ...
  latex.DrawLatex(0.5*(leg_x1+leg_x2),leg_y1-0.05,"Anti k_{T} CHS");
  ...
  for(int fg=0; fg<nfiles; fg++){
      ...
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("MC/Nominal");
      ...
      tdrDraw(hist_radius[fg][ij][jk],algo_style[fg],col[fg],line_sty[fg],col[fg] ,1001,2);
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
   ...
   hist_radius_rat[fg][ij][jk]->SetLineStyle(line_sty[fg+1]);
   hist_radius_rat[fg][ij][jk]->SetMarkerColor(col[fg+1]);
   hist_radius_rat[fg][ij][jk]->SetMarkerStyle(algo_style[fg+1]);
   if(fg==0){
  ...
  sprintf(name,"ISR_FSR_Plots/Ratio_Plots/%s.pdf",c3[ij][jk]->GetName());
