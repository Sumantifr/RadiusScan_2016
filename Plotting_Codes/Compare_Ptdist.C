...
const char *filenames[nfiles] =
   {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Data_2016/Legacy/JECv18/Legacy2016_07Aug_JECV18_TPCor_excor.root",
...
//	"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Pythia_NTuples_LegacyCor_JECv15_new_excor.root",
	"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Pythia_NTuples_LegacyCor_wRESO_excor.root",
	"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Madgraph/NTuples_Files/Histograms_Madgraph_Summer_w50_excor.root"};
...
*/
int mstyle[nfiles] = {kFullCircle,kFullTriangleUp,kFullTriangleDown,kOpenDiamond};
double data_lumi = 1.;
...
const char *model_name[nfiles] = {"Data","Herwig++(CUETHppS1)","Pythia8(CUETP8M1)","Madgraph+P8(CUETP8M1)"};
const int col[nfiles] = {1,4,2,kGray+1};
...
//     if(fg!=0) {hist_pt_org[fg][ij][jk]->Scale(hist_pt_org[0][ij][jk]->Integral()*1./hist_pt_org[fg][ij][jk]->Integral()); }
   ...
   for(int bn=0; bn<(hist_pt[fg][ij][jk]->GetNbinsX()); bn++){
     hist_pt[fg][ij][jk]->SetBinContent(bn+1,hist_pt_org[fg][ij][jk]->GetBinContent(bn+1)*1.0/(hist_pt[fg][ij][jk]->GetBinWidth(bn+1)*2*(ybin[jk+1]-ybin[jk])));
     hist_pt[fg][ij][jk]->SetBinError(bn+1,hist_pt_org[fg][ij][jk]->GetBinError(bn+1)*1.0/(hist_pt[fg][ij][jk]->GetBinWidth(bn+1)*2*(ybin[jk+1]-ybin[jk])));
    }
    ...
    hist_pt[fg][ij][jk]->SetMaximum(10*hist_pt[fg][ij][jk]->GetMaximum());
...
for(int ij=0; ij<nalgo; ij++){
  for(int jk=0; jk<netarange; jk++){ 
    for(int pt=0; pt<noptbins; pt++){
     hist_pt_data[ij][jk]->SetBinContent(pt+1,hist_pt[0][ij][jk]->GetBinContent(pt+1));
     hist_pt_data[ij][jk]->SetBinError(pt+1,hist_pt[0][ij][jk]->GetBinError(pt+1));
    }
  }
}
...
leg_pt[ij][jk] = tdrLeg(0.445,0.75,0.795,0.95);
...
c2[ij][jk]->cd(1);
 ...
 gPad->SetLogy(1);
  ...
  latex.DrawLatex(0.5*(leg_x1+leg_x2)+0.1,leg_y1+0.025,"Anti-k_{T}");
...
//	hist_pt[fg][ij][jk]->Scale(1./hist_pt[fg][ij][jk]->Integral());
      ...
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetLabelSize(0.1);
      ...
      hist_pt_rat[fg-1][ij][jk]->GetXaxis()->SetNoExponent();
      ...
      tdrDraw(hist_pt[fg][ij][jk],mstyle[fg],col[fg],kSolid,col[fg] ,1001,2);
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
  ...
  hist_pt_rat[fg][ij][jk]->SetMarkerStyle(mstyle[fg+1]);
  hist_pt_rat[fg][ij][jk]->SetMarkerColor(col[fg+1]);
//  hist_pt_rat[fg-1][ij][jk]->SetMarkerSize(2);
  hist_pt_rat[fg][ij][jk]->GetXaxis()->SetRangeUser(min_range,max_range);
if(fg==0) {  hist_pt_rat[fg][ij][jk]->Draw("ep"); }
 else { hist_pt_rat[fg][ij][jk]->Draw("sames"); }
 }

 line->Draw();

sprintf(name,"Pt_Plots/Legacy_v18/No_TPCor/%s_thesis.pdf",c2[ij][jk]->GetName());
...
//  if((ij!=ieta)|(jk!=ialgo)) continue;
  ...
  latex.DrawLatex(0.5*(leg_x1+leg_x2),leg_y1-0.05,"Anti k_{T}");
  latex.DrawLatex(0.5*(leg_x1+leg_x2)-0.05,leg_y1-0.125,text);
  latex.DrawLatex(0.5*(leg_x1+leg_x2)-0.05,leg_y1-0.2,text1);
  ...
  for(int fg=0; fg<nfiles; fg++){
      ...
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetLabelSize(0.1);
      ...
      hist_radius_rat[fg-1][ij][jk]->SetMarkerStyle(mstyle[fg]);
      hist_radius_rat[fg-1][ij][jk]->SetMarkerSize(2);
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
  ...
  sprintf(name,"Ratio_Plots/Legacy_v18/No_TPCor/%s_thesis.pdf",c3[ij][jk]->GetName());
