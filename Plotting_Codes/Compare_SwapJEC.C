...
const char *filenames[nfiles] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Historam_P8_Noswap.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Histograms_Pythia8_JECSwap.root"};
...
const char *model_name[nfiles] = {"P8_Nominal","P8_JEC_Swapped"};
const int col[nfiles] = {1,6};
...
c2[ij][jk]->cd(1);
 ...
 for(int fg=0; fg<nfiles; fg++){
      ...
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("JEC Swap / Nominal");
      hist_pt_rat[fg-1][ij][jk]->GetYaxis()->SetTitleSize(0.05);
	}
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
  ...
  hist_pt_rat[fg][ij][jk]->SetMinimum(0.75);
  hist_pt_rat[fg][ij][jk]->SetMaximum(1.25);
...
sprintf(name,"JEC_Swap/Pt_Plots/%s.eps",c2[ij][jk]->GetName());
...
//  if((ij!=ieta)|(jk!=ialgo)) continue;
  ...
  for(int fg=0; fg<nfiles; fg++){
      ...
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitle("JEC Swap / Nominal");
      hist_radius_rat[fg-1][ij][jk]->GetYaxis()->SetTitleSize(0.05);

      hist_radius_rat[fg-1][ij][jk]->SetMinimum(0.75);
      hist_radius_rat[fg-1][ij][jk]->SetMaximum(1.25);
...
//tdrDraw(hist_pt_rat[fg-1][ij][jk],kFullCircle,col[fg],kSolid,col[fg] ,1001,2);
  ...
  sprintf(name,"JEC_Swap/Ratio_Plots/%s.eps",c3[ij][jk]->GetName());
