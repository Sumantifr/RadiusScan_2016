...
double ptbins[noptbins+1] = {/*30,37, 43, 49, 56, 64,*/74, 84, 97, 114, 133, 153, 174, 196, 220, 245, 272, 300, 330, 362, 395, 430, 468, 507, 548, 592, 638, 686, 737, 790, 846, 905, 967, 1032, 1101, 1172, 1248, 1327, 1410, 1497, 1588, 1684, 1784, 1890, 2000, 2116, 2238, 2366, 2500, 2640, 2787, 2941, 3103, 3273, 3450, 3637, 3832, 4037, 4252, 4477, 4713, 4961, 5220, 5492, 5777, 6076, 6389, 6717, 7000};
...
const int bayes_par = 8;
const int svd_par = 9;

float data_lumi = 27.681;
...
TH1D* rebin_1D(TH1D *thin, int lastbin) {

//lastbin = nbins[lastbin];
...
thout = new TH1D(namey, titley, lastbin, xmod2);

for(int bn=0; bn<(lastbin+1); bn++){
  thout->SetBinContent(bn+1,zval[bn+1]);
  thout->SetBinError(bn+1,zerr[bn+1]);
 }

return thout;
}

TH2D* rebin_hist2d(TH2D *thin, TH1D *th1, int lastbin) {

TH2D *thout;

int nibis_new = lastbin;

double xmod2[100]={0};
double ymod2[100]={0};

int ifirst  = 1;
int nbinsx_new = nibis_new;//thin->GetNbinsX();
int nbinsy_new = nibis_new;//thin->GetNbinsY();

cout<<"nbinsX "<<nbinsx_new<<" nbinsY "<<nbinsy_new<<endl;

for(int ix=ifirst; ix<=(nbinsx_new+1); ix++){
xmod2[ix-1] = th1->GetBinLowEdge(ix);
}
for(int iy=ifirst; iy<=(nbinsy_new+1); iy++){
ymod2[iy-1] = th1->GetBinLowEdge(iy);
}

sprintf(namey,"Roo_%s",thin->GetName());
sprintf(titley,"Roo_%s",thin->GetTitle());

thout = new TH2D(namey,titley,nbinsx_new,xmod2,nbinsy_new,ymod2);

for(int ix=0; ix<=(thin->GetNbinsX())/*nbinsx_new*/; ix++){
 for(int iy=0; iy<=(thin->GetNbinsY())/*nbinsy_new*/; iy++){
   if((ix==0&&iy==0)||(ix==(thin->GetNbinsX())&&iy==(thin->GetNbinsY()))) continue;
   if((ix<=nbinsx_new)&&(iy<=nbinsy_new)){
   thout->SetBinContent(ix,iy,thin->GetBinContent(ix,iy));
   thout->SetBinError(ix,iy,thin->GetBinError(ix,iy));
	}
  else{
//      thout->Fill(thin->GetXaxis()->GetBinCenter(ix),thin->GetYaxis()->GetBinCenter(iy)); 
   }
 }
}
return thout;
...
//cout<<hist_back->GetName()<<" Sum 5th row "<<sum<<" eff "<<eff[5]<<endl;
}
...
void  Unfolding_trial_Allhist_TUnfold()
{	

double integral_ratio = 1.;

TFile *fileout ;
fileout = new TFile("Unfolded_2016_Data_TUnfold_new.root","recreate") ;

//const char* rootfiles[3] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Data_2016/NTuples/2016BCDEFGH_ntuple_nocor.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/P8_FullPU_tworeso.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/P8_FullPU_tworeso.root"};

const char* rootfiles[3] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Data_2016/RootFiles/2016Files/hist_RecoJetchs_PU_Data_JetHT2016BCDEFGH.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/P8_FullPU_tworeso.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/P8_FullPU_tworeso.root"};
...
for(int ij=0; ij<nalgo; ij++){
 for(int jk=0; jk<Etabins; jk++){
  ...
  hist_tuf[ij][jk] = new TH1D(name,name,noptbins,ptbins) ;
  ...
  sprintf(name,"Tuf_%s_EtaBin%i_back",JetRad[ij],jk+1);
  hist_tuf_back[ij][jk] = new TH1D(name,name,noptbins,ptbins) ;
  ...
  sprintf(name,"Tuf_%s_EtaBin%i_error",JetRad[ij],jk+1);
  hist_tuf_error[ij][jk] = new TH1D(name,name,noptbins,1,noptbins);
...
//	sprintf(name,"JetpT_%s_EtaBin%i",JetRad[jk],kl+1);
	...
	lastbin[jk][kl] = LastBin_Counter(hist_data[jk][kl]);
        cout<<name<<" LastBin1 "<<lastbin[ij][jk]<<endl;
if(ij==1) { 
	sprintf(name,"JetpT_%s_EtaBin%i",JetRad[jk],kl+1);
...
//	mat_rm[jk][kl] = rebin_hist2d(mat_rm_b[jk][kl],hist_gen[jk][kl],lastbin[jk][kl]);
	}
   }//kl
  }//jk
}//ij
...
//    hist_data2[ij][jk] = rebin_1D(hist_data[ij][jk],lastbin[ij][jk]);
    hist_data2[ij][jk]->Rebin(2);
    hist_data3[ij][jk] = (TH1D*)hist_data[ij][jk]->Clone();
...
RooUnfoldBayes unfoldBayes(&response_b,hist_input,bayes_par,false);
...
//std::cout<<"chi**2="<<unfoldtuf.GetChi2A()<<"+"<<unfoldtuf.GetChi2L()<<" / "<<unfoldtuf.GetNdf()<<"\n";

unfoldtuf.DoUnfold(0.1);
...
for(int bn=0; bn<hist_gen2[ij][jk]->GetNbinsX(); bn++){
cout<<hist_gen2[ij][jk]->GetBinLowEdge(bn+1)<<",";
}

unfoldtuf.GetOutput(hist_tuf[ij][jk],binMap);

cout<<" optimum tau is "<<unfoldtuf.GetTau()<<endl;
...
/*
for(int bn=0; bn<(hist_tuf[ij][jk]->GetNbinsX()); bn++){
hist_tuf[ij][jk]->SetBinContent(bn+1,(hist_tuf[ij][jk]->GetBinContent(bn+1))*1./eff[bn]);
hist_tuf[ij][jk]->SetBinError(bn+1,(hist_tuf[ij][jk]->GetBinError(bn+1))*1./eff[bn]);
}
*/
...
unfoldtuf.GetFoldedOutput(hist_tuf_back[ij][jk],binMap);

for(int bn=0; bn<(hist_tuf_back[ij][jk]->GetNbinsX()); bn++){
hist_tuf_back[ij][jk]->SetBinContent(bn+1,hist_tuf_back[ij][jk]->GetBinContent(bn+1)*1./(1.-fakerate[bn])) ;
}
...
hist_bayes_back[ij][jk]->Divide(hist_data3[ij][jk]);
hist_svd_back[ij][jk]->Divide(hist_data3[ij][jk]);
hist_BbB_back[ij][jk]->Divide(hist_data3[ij][jk]);
hist_tuf_back[ij][jk]->Divide(hist_data3[ij][jk]);
...
hist_tuf[ij][jk]->Divide(hist_bayes[ij][jk]);
*/
...
data_bayes_diff[bn] = (hist_bayes_back[ij][jk]->GetBinContent(bn+1)-1.)*hist_data3[ij][jk]->GetBinContent(bn+1) ;
data_svd_diff[bn] = (hist_svd_back[ij][jk]->GetBinContent(bn+1)-1.)*hist_data3[ij][jk]->GetBinContent(bn+1) ;
 }
else{
...
for(int am=0; am<lastbin[ij][jk]; am++){
 for(int bn=0; bn<lastbin[ij][jk]; bn++){
  if(am==bn){
  data_errmat[am][bn] = pow(hist_data3[ij][jk]->GetBinError(am+1),2.);
	}
...
double rel_error_data = ( hist_data2[ij][jk]->GetBinContent(bn+1) > 1.e-7) ? hist_data2[ij][jk]->GetBinError(bn+1) *1./ hist_data2[ij][jk]->GetBinContent(bn+1): 1.e-7 ;
...
double rel_error_tuf = (hist_tuf[ij][jk]->GetBinContent(bn+1)>1.e-7) ? hist_tuf[ij][jk]->GetBinError(bn+1)*1./hist_tuf[ij][jk]->GetBinContent(bn+1) : 1.e-7 ;
...
hist_tuf_error[ij][jk]->SetBinContent(bn+1,rel_error_tuf*1./rel_error_data);
hist_tuf_error[ij][jk]->SetBinError(bn+1,0.);
 }
}//bn
...
int main()
{
Unfolding_trial_Allhist_TUnfold();
}
