...
TFile *responsematrfile = new TFile("Response_Matrix/Spline_output_total.root","read");
...
void fill_histogramme(int lm, int jk, int mn) {
...
if(lm==0){
  ...
  if (lm==indexx) { //2d only from PFjet
    ...
    sprintf(histname,"Response_Matrix_%s_EtaBin%i",JetRad[jk],mn+1);//"RecoJetvsGenJet_Algo%i_EtabBin%i",jk+1,mn+1);
    
    h2d_mccorrel =/* (TH2D*)fileInput[lm]->Get(histname) ;*//*rebin_hist2d(*/(TH2D*)responsematrfile->Get(histname);//, 0, kl);
 
    double sum[62] = {0.} ;
    for(int bnx=0; bnx<(h2d_mccorrel->GetNbinsX());bnx++){
		for(int bny=0; bny<(h2d_mccorrel->GetNbinsY()); bny++){
		sum[bnx]+= h2d_mccorrel->GetBinContent(bnx+1,bny+1);
		}
		for(int bny=0; bny<(h2d_mccorrel->GetNbinsY()); bny++){
		h2d_mccorrel->SetBinContent(bnx+1,bny+1,h2d_mccorrel->GetBinContent(bnx+1,bny+1)*1./sum[bnx]);
     	}
	}
 
 // h2d_mccorrel->Scale(1./sum);
  for(int bnx=0; bnx<(h2d_mccorrel->GetNbinsX());bnx++){
	  for(int bny=0; bny<(h2d_mccorrel->GetNbinsY());bny++){
		//  h2d_mccorrel->SetBinContent(bnx+1,bny+1,h2d_mccorrel->GetBinContent(bnx+1,bny+1)*1./10000) ;
		  cout<<bnx<<'\t'<<bny<<'\t'<<h2d_mccorrel->GetBinContent(bnx+1,bny+1)<<endl;
		  }
	  }
    cout<<JetRad[jk]<<" "<<mn<<" integral "<<h2d_mccorrel->Integral()<<endl;
...
int main()
{
  ...
  for(int ij=2; ij<3/*nalgo*/; ij++){
   for (int mn=0; mn<1/*netarange*/; mn++) {
	   ...
	   int natype=0; //fill Geninformation only once for four algorithm
		...
		for (int lm=0; lm<nfiles; lm++) {
			fill_histogramme(lm,ij,mn);
           if(lm==0) { hist_data[ij][mn] = (TH1F*)h_recoinput[lm]->Clone(); }
		}
      ...
      sprintf(titley, "unfold %s", h_recoinput[indexx]->GetTitle());
	  ...
	  for (int ix=0; ix<h2d_mccorrel->GetNbinsX(); ix++) {
	    for (int iy=0; iy<h2d_mccorrel->GetNbinsY(); iy++) {
	      ...
	      tmatrdd(ix,iy) = tmatr(ix,iy)/scal; //GMA
	    }
	  }
	  ...
	  if (lm>=0) { // Not only for data
		...
		RooUnfoldSvd unfoldsvd(&response, h_recoinput[lm], svdsft); 
