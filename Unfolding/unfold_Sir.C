...
TFile *responsematrfile = new TFile("Response_Matrix/Spline_output_10M.root","read");
...
const char* rootname[nfiles]={"../CPlusPlus/CMS_Files/Jets_in_CMS/Data_2015D/hist_RecoJetchs_PU_Data_2015D_JetHTCHS_NewPrescale_v2.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Madgraph/Madgraph_MCJets_CHS_syst.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/hist_Pythia_pTBinnedCHS_LumiWeighted_syst.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Herwig/Herwig_Flat_Proper_Weight.root"};
...
TFile *ResponseMatrix_file = new TFile("Response_Matrix/Spline_output_10M.root","read");
...
void hist_setbins(TH1D* th, int lm) {
  ...
  double dx=0.;//0.2*(nfiles/2.-lm)*th->GetBinWidth(nbn/2);
  double xval[nbmx];
...
void fill_histogramme(int lm, int jk, int mn, int nmcfill) {
...
if(lm==0){
  ...
  sprintf(histname,"mcjets/JetpT_%s_EtaBin%i",JetRad[jk], mn+1);
  }
else {
  ...
  h_recoinput[lm] = rebin_hist(tmphist2x,mn);
  
  if(lm==0&&jk==0&&mn==0) {
	  cout<<"lowedges \n";
	  for(int ix=0; ix<tmphist2x->GetNbinsX(); ix++){
		  cout<<tmphist2x->GetBinLowEdge(ix)<<'\t';
		  }
  }
  ...
  if (lm>0 && nmcfill==0) { //Only for MC and and only once for two jet types
    ...
    h_mcgeninput[lm] = (TH1D*)tmphist1x->Clone();/*_hist(tmphist1x,mn);*/
  }
  //Response matrix;

  if (lm==indexx) { //2d only from PFjet

 //   ResponseMatrix_file->cd();   
 
    sprintf(histname,/* "Response_Matrix_AK%i_EtaBin%i",jk+2,mn+1);*/"RecoJetvsGenJet_Algo%i_EtabBin%i",jk+1,mn+1);
    
    cout<<jk<<" "<<lm<<" "<<histname<<endl;
    
    h2d_mccorrel = rebin_hist2d((TH2D*)fileInput[lm]->Get(histname));//, 0, kl);
    
  //  h2d_mccorrel->Scale(1./h2d_mccorrel->Integral());
...
void fill_genhistogramme(int lm, int kl, int mn) {
  ...
  h_mcgeninput[nfiles] = rebin_hist(tmphist1x/*, kl*/,mn);
...
int main()
{
  ...
  ofstream fpmat;
  fpmat.open("Svd_error_matrix.log");
  
  ofstream fpdgn;
  fpdgn.open("svd_error_diagonal.log");
  ...
  sprintf(evname,"%s","ratio_unfbydata");
  sprintf(padmcname,"%s","ratio_mc");
  sprintf(odname,"%s","Pt_spec");
  
  
  for(int ij=2; ij<3/*nalgo*/; ij++){
...
////	  natype++;
	...
	//  int isub = subtract_background(h2d_mccorrel,  h_recoinput[0], h_recoinput[1], h_recoinput[2], h_recoinput[3], fakerate, efficiency, 0);
	  ...
	  for(int ix=0;ix<h_recoinput[0]->GetNbinsX();ix++){
		  cout<<h_recoinput[0]->GetBinLowEdge(ix)<<'\t';
		  }
	  cout<<endl;
  }
...
////OR
   /////    iend=nmxxiter; start=iend-1;
	  ...
	  if (lm>=0) { // Not only for data
		...
		RooUnfoldBayes unfoldbayes(&response, h_recoinput[lm], /*ijk*/4, false);
	...
	//	cout<<"h_recoinput[lm] integral "<<h_recoinput[lm]->Integral()<<endl;
		...
		if(lm==0&&ij==0&&mn==0){
			cout<<"after svd unfolding lm "<<lm<<endl;
			for(int ix=0; ix<h_unfoldedbayes[lm]->GetNbinsX();ix++){
				cout<<h_unfoldedbayes[lm]->GetBinLowEdge(ix)<<endl ;
				}
			}
	...
	//	h_unfoldedsvd[lm] ->Draw() ;
		...
		sprintf(namey, "bayes_%s", modelname[lm]/*, JetRad[ij], ijk*/);
		add_nameindex(h_unfoldedbayes[lm], namey);

		sprintf(namey, "svd_%s_", modelname[lm]/*, JetRad[ij], svdsft*/); //ijk+svdsft); //svdsft; //ijk+svdsft
	...
	//	covmatrixbayes[lm] = unfoldbayes.GetPropCov();
       ...
       // fpmat<<"svdfile "<<lm<<" ak"<<ij+2<<" etabin"<<mn+1<<endl;
        
     if(lm==0){
        for(int cm=0 ; cm<nbin_eta[mn];cm++){
			for(int dn=0; dn<nbin_eta[mn];dn++){
			fpmat<<covmatrixsvd[lm](cm,dn)/(max(1.e-6,h_unfoldedsvd[lm]->GetBinContent(cm+1))*max(1.e-6,h_unfoldedsvd[lm]->GetBinContent(dn+1)))<<'\t';
				}
				cout<<'\n';
			}
			
		for(int cm=0; cm<nbin_eta[mn]; cm++){
			fpdgn<<cm<<'\t'<<covmatrixsvd[lm](cm,cm)*1.0/(max(1.e-6,h_unfoldedsvd[lm]->GetBinContent(cm+1))*max(1.e-6,h_unfoldedsvd[lm]->GetBinContent(cm+1)))<<endl; ;
			}	
	       } //lm==0
	...
	//	rathist1[lm][0]->SetTile("");
		...
		rathist1[lm][0]->GetXaxis()->CenterTitle();
	...
	//	rathist3[lm][0]->Scale(1./rathist3[lm][0]->Integral()); //open to normalize
	...
	//	  rathist6[lm][0]->SetTile("");
		  ...
		  rathist6[lm][0]->GetXaxis()->CenterTitle();
	...
	//	rathist5[lm][0]->Scale(1./rathist5[lm][0]->Integral()); //open to normalize
		rathist5[lm][0]->SetMaximum(1.18*rathist5[lm][0]->GetMaximum());
		hist_setbins(rathist5[lm][0], lm);
		
		if(lm==0&&ij==0&&mn==0){
			cout<<"in pad 3\n";
			for(int ix=0; ix<rathist5[lm][0]->GetNbinsX(); ix++){
				cout<<rathist5[lm][0]->GetBinLowEdge(ix)<<" before "<<h_unfoldedsvd[lm]->GetBinLowEdge(ix)<<endl ;
				}
			}
	...
	//	rathist5[lm][0]->SetTile("");
		...
		rathist5[lm][0]->GetXaxis()->CenterTitle();
...
//		  rathist7[lm][0]->SetMaximum(1.2); rathist7[lm][0]->SetMinimum(0.8); 
		...
		//  rathist8[lm][0]->SetTile("");
		  ...
		  rathist8[lm][0]->GetXaxis()->CenterTitle();
		...
		if (isMC) {
		  ...
		  sprintf(namey,"%s","error");
		  add_nameindex(rathist9[lm][0],namey);
		  ...
		  add_nameindex(rathist10[lm][0],namey);
