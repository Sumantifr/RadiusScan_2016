...
void fill_histogramme(int lm, int jk, int mn, int nmcfill,int jer) {
...
if(lm==0){
  ...
  sprintf(histname,"JetpT_%s_EtaBin%i_JER%i",JetRad[jk], mn+1,jer); 
  }
...
int main()
{
  ...
  sprintf(titley, "unfold_systematics_JER");  
   ...
   /////    iend=nmxxiter; start=iend-1;
	...
	//	covmatrixbayes[lm] = unfoldbayes.GetPropCov();
		...
		double deltax2[3]={0.,0.,0.};
		  ...
		  sprintf(evname,"%s_JER%i","ratio_unfbydata",jk);
		  sprintf(padmcname,"%s_JER%i","ratio_mc",jk);
          sprintf(odname,"%s_JER%i","Pt_spec",jk);
