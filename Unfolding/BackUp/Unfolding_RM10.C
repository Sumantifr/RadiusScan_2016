...
int nbiny = h2d_correl->GetNbinsY();
 ...
 for (int iy=0; iy<nbiny; iy++) {
  //    cout<<iy<<" InEff "<<effi[iy]<<" Total GEN "<<totalgen[iy+1]<<endl;
      effi[iy] = (totalgen[iy+1] - effi[iy])/max(1.e-10, totalgen[iy+1]);
  //    cout<<iy<<" Eff "<<effi[iy]<<" Total GEN "<<totalgen[iy+1]<<endl;
...
//	cout<<ix<<" FAke "<<fakerate[ix]<<" Total RECO "<<totalreco[ix+1]<<endl;
...
TFile *fileout ;
fileout = new TFile("Unfolded2016_Stat10.root","recreate") ;

//const char* rootfiles[3] = {"/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/Data_2016/RootFiles/2016Files/hist_RecoJetchs_PU_Data_JetHT2016BCDEFGH.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Hist_P8_RM10_Combo.root","/home/suman/CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Pythia/NTuples_Hist/Hist_P8_RM10_Combo.root"};
...
//	sprintf(name,"mcjets/JetpT_%s_EtaBin%i",JetRad[jk],kl+1);
	...
	hist_data[jk][kl][lm]->Scale(1./data_lumi);
	}
	}else{
 		hist_data[jk][kl][lm] = (TH1D*)hist_data[jk][kl][0]->Clone();
	    }*/
	    sprintf(name,"JetpT_%s_EtaBin%i_Comb%i",JetRad[jk],kl+1,lm+1);
	    hist_data[jk][kl][lm] = (TH1D*)file1->Get(name);
		hist_data[jk][kl][lm]->Scale(10./data_lumi);
	}
if(ij==1) {
	sprintf(name,"JetpT_%s_EtaBin%i",JetRad[jk],kl+1);
...
for(int ij=0; ij<nalgo; ij++){
 ...
 //  if(ij==3&&jk==0) { cout<<"data comb "<<kl+1<<" mean "<<hist_data[ij][jk][kl]->GetMean()<<endl; }
...
for(int ij=0; ij<nalgo; ij++){
 for(int jk=0; jk<Etabins; jk++){
  for(int kl=0; kl<ncomb; kl++){
  cout<<"mean "<<JetRad[ij]<<" etabin"<<jk+1<<" comb "<<kl+1<<" data "<<hist_data[ij][jk][kl]->GetMean()<<" gen "<<hist_gen[ij][jk][kl]->GetMean()<<" bayes "<<hist_bayes[ij][jk][kl]->GetMean()<<" svd "<<hist_svd[ij][jk][kl]->GetMean()<<" BbB "<<hist_BbB[ij][jk][kl]->GetMean()<<endl;
 cout<<"std dev "<<JetRad[ij]<<" etabin"<<jk+1<<" comb "<<kl+1<<" data "<<hist_data[ij][jk][kl]->GetStdDev()<<" gen "<<hist_gen[ij][jk][kl]->GetStdDev()<<" bayes "<<hist_bayes[ij][jk][kl]->GetStdDev()<<" svd "<<hist_svd[ij][jk][kl]->GetStdDev()<<" BbB "<<hist_BbB[ij][jk][kl]->GetStdDev()<<endl;
  }
 }
}
*/
