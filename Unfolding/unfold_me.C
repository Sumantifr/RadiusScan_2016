...
const char* genrootname[ngenfiles]={"gen_qcdevt_herwigpp_fall11_all_redgen2.root"}; //, "../2012/test_pythia8_redbin.root"};
const char* genmodelname[ngenfiles]={"gen_Herwig"}; //, "gen_pythia8"};

const int nfiles=5;

const char* rootname[nfiles]={"../CPlusPlus/CMS_Files/Jets_in_CMS/Data_2015D/hist_RecoJetchs_PU_Data_2015D_JetHTCHS_NewPrescale_v2.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Madgraph/Madgraph_MCJets_CHS_GenFull.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Madgraph/Madgraph_MCJets_CHS_GenFull.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Madgraph/Madgraph_MCJets_CHS_GenFull.root", "../CPlusPlus/CMS_Files/Jets_in_CMS/GenJets/Madgraph/Madgraph_MCJets_CHS_GenFull.root"};
 
//const char* modelname[nfiles]={"Data", "Pythia6", "Pythia8", "Herwig", "Madgraph"};
const char* modelname[nfiles]={"Madgraph","Madgraph","Madgraph","Madgraph","Madgraph"};
...
TH1D* rebin_hist(TH1D* thin/*, int ijetpt*/) {
...
if(lm==0){
  ...
  if (lm>0 && nmcfill==0) { //Only for MC and and only once for two jet types
    ...
    h_mcgeninput[lm] = (TH1D*)/*tmphist1x->Clone();*/rebin_hist(tmphist1x);
  }
  //Response matrix;

  if (lm==1) { //2d only from PFjet
...
void fill_genhistogramme(int lm, int mn, int kl) {
  // lm : input file number
  // ij : jetptbn
  // mn : jetetamn
  ...
  h_mcgeninput[nfiles] = (TH1D*)/*tmphist1x->Clone();*/rebin_hist(tmphist1x/*, kl*/);
/*  
...
int main()
{
  ...
  sprintf(titley, "unfold_xxxxxx");  
  sprintf(namey, "outfile_%s.root", titley);
  ...
  for (int ij=0; ij<ngenfiles-1; ij++) {
  ...
  for(int ij=0; ij<nalgo; ij++){
   ...
   //            fileOut->cd();
		...
		for (int lm=0; lm<nfiles; lm++) {
		 ...
		 if (jk==0) {
	    for (int lm=0; lm<ngenfiles-1; lm++) {
	         fill_genhistogramme(1,mn,ij);
	    } 
	  }
////	  natype++;
...
////OR
   /////    iend=nmxxiter; start=iend-1;
	   ...
	   for (int ijk=start; ijk<iend; ijk++) {
	    ...
	    for (lm=0; lm</*nfiles*/1; lm++) {
			
			bool isData = (lm==0) ? true : false;
	  ...
	  if (lm>=0) { // Not only for data
		...
		RooUnfoldBayes unfoldbayes(&response, h_recoinput[lm], /*ijk*/2, false);
	...
	//	h_unfoldedsvd[lm] ->Draw() ;
		...
		sprintf(namey, "bayes_%s_%s_%i", modelname[lm], JetRad[ij], ijk);
		add_nameindex(h_unfoldedbayes[lm], namey);

		sprintf(namey, "svd_%s_%s_%i", modelname[lm], JetRad[ij], svdsft); //ijk+svdsft); //svdsft; //ijk+svdsft
	...
	//	covmatrixbayes[lm] = unfoldbayes.GetPropCov();
		...
		if (icol > 1) {
		  ...
		  rathist2[lm][0]->SetMaximum(1.6); rathist2[lm][0]->SetMinimum(0.6); 
	...
	/////	  calc_deltax_cov(deltax2, h_unfoldedbayes[lm],  h_unfoldedbayes[!isData], covmatrixbayes[lm], covmatrixbayes[!isData]);  //need to be open
		  rathist4[lm][0]->SetMaximum(1.6); rathist4[lm][0]->SetMinimum(0.6); 
		...
		//  file_out<<"6name "<< rathist6[lm][0]->GetName()<<" "<<rathist6[lm][0]->GetTitle()<<endl;
		  ...
		  rathist6[lm][0]->SetMaximum(1.6); rathist6[lm][0]->SetMinimum(0.6); 
		...
		/////  file_out<<"7name "<< rathist7[lm][0]->GetName()<<" "<<rathist7[lm][0]->GetTitle()<<endl;
		  ...
		  rathist7[lm][1] = (TH1D*)h_mcgeninput[lm]->Clone();
